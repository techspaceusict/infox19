...
pointers2.push( new pointerPrototype() );
...
function update() {
    ...
    for ( var i = 0, len =  pointers2.length; i < len; i++ ) {
        var pointer = pointers2[ i ];
...
function shoot(shiftX,shiftY){
  
    count++;

    ( count > 25 ) && (colorArr = [ Math.random() + 0.2, Math.random() + 0.2, Math.random() + 0.2 ], count = 0);
    let midWidth=window.innerWidth/2+shiftX,midHeight=window.innerHeight/2+shiftY,
        midWidth2=midWidth-2*shiftX,midHeight2=midHeight-2*shiftY;
  
    pointers[ 0 ].down  = true;
    pointers[ 0 ].color = colorArr;
    pointers[ 0 ].moved = pointers[ 0 ].down;
    pointers[ 0 ].dx    = (midWidth - pointers[ 0 ].x) * 10.0;
    pointers[ 0 ].dy    = (midHeight - pointers[ 0 ].y) * 10.0;
    
    pointers[ 0 ].x     = midWidth;
    pointers[ 0 ].y     = midHeight;
  
    pointers2[ 0 ].down  = true;
    pointers2[ 0 ].color = colorArr;
    pointers2[ 0 ].moved = pointers2[ 0 ].down;
    pointers2[ 0 ].dx    = (midWidth2 - pointers2[ 0 ].x) * 10.0;
    pointers2[ 0 ].dy    = (midHeight2 - pointers2[ 0 ].y) * 10.0;
    
    pointers2[ 0 ].x     = midWidth2;
    pointers2[ 0 ].y     = midHeight2;
}
// setTimeout(()=>{
//   for(var i=0; i<50; i++) shoot(0)
//   for(var i=0; i<50; i++) shoot(0);
// },1000);
// setTimeout(()=>{
//   for(var i=0; i<)shoot(50);
// },1000)
var howManyTimes=50,i=0;
function startShoot(val) {
    i++;
    if( i < howManyTimes ){
        setTimeout(()=>{
          startShoot(val);
